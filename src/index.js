...
function createNewBot(botNumber = 1, useNewIdentity = false, customName = null, customUUID = null) {
  ...
  bot.once('spawn', () => {
    ...
    bot.on('entityHurt', (entity) => {
      ...
      const attacker = recentDamage.attacker || bot.nearestEntity(e => 
      ...
      if (attacker && !currentTarget) {
        ...
        addGameLog(`💥 Attacked by ${attackerName}`, botNumber);
    ...
    <head>
      ...
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .bot-remove { background: #7f1d1d; color: white; border: none; padding: 0.25rem 0.5rem; border-radius: 0.25rem; font-size: 0.7rem; cursor: pointer; }
        ...
        /* Stats cards */
        .stats-cards { display: grid; grid-template-columns: repeat(2, 1fr); gap: 0.5rem; margin-bottom: 1rem; }
            ...
            .then(data => {
              ...
              data.bots.forEach(bot => {
                ...
                botDiv.innerHTML = \`
                  ...
                  <div class="bot-stats">
                    \${bot.online ? \`
                      <span class="bot-health" title="Health">♥\${bot.health}</span>
                      <span class="bot-food" title="Food">🍗\${bot.food}</span>
                    \` : ''}
                    <button class="bot-remove" onclick="removeBot(\${bot.id})" title="Remove bot">X</button>
                  </div>
                \`;
            ...
            .then(data => {
              ...
              data.bots.forEach(bot => {
                ...
                botDiv.innerHTML = \`
                  ...
                  <div>
                    <button class="btn btn-small btn-danger" onclick="removeBot(\${bot.id})">Remove</button>
                  </div>
                \`;
            ...
            .then(data => {
              ...
              if (data.logs.length > 0) {
                ...
                data.logs.forEach(log => {
                  ...
                  if (log.includes('Attacked') || log.includes('Fighting')) logClass = 'log-combat';
...
app.post('/api/update-server', (req, res) => {
  ...
  // Clear allBots but keep identities
  allBots.clear();

  // Reconnect all bots to new server
  setTimeout(() => {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  if (initialBots > 0) {
    ...
    for (let i = 1; i <= initialBots; i++) {
      setTimeout(() => createNewBot(i, false), i * 1000);
    }
  }
});
